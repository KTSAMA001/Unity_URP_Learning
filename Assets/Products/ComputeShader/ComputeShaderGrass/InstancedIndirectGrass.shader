...
float3 _OffsetPos;
            ...
            Varyings vert(Attributes IN, uint instanceID : SV_InstanceID)
            {
                ...
                float4 perGrassPivotPosWS =  mul(_GrassInfoBuffer[instanceID].TRS,IN.positionOS)+float4(_OffsetPos,0);;//we pre-transform to posWS in C# now
perGrassPivotPosWS /= perGrassPivotPosWS.w;
              ...
              //  positionOS = lerp(positionOS.xyz + bendDir * positionOS.y / -bendDir.y, positionOS.xyz, stepped * 0.95 + 0.05);//don't fully bend, will produce ZFighting
                float3  positionOS = mul(unity_WorldToObject,perGrassPivotPosWS);//don't fully bend, will produce ZFighting

                //per grass height scaledx
               ...
               // positionOS += cameraTransformRightWS * IN.positionOS.x * max(0, ViewWSLength * 0.0225);
                ...
                float3 positionWS = perGrassPivotPosWS;
