...
// Computes the specular term for EnvironmentBRDF
half3 EnvironmentBRDFSpecular(BRDFData brdfData, half fresnelTerm,half NV)
{
    float3 F = FresnelSchlickRoughness(NV, brdfData.specular, brdfData.perceptualRoughness);
    //数值近似
    float2 env_brdf = EnvBRDFApprox(brdfData.perceptualRoughness,NV);
    float surfaceReduction = 1.0 / (brdfData.roughness2 + 1.0);
    return (F * env_brdf.r + env_brdf.g);
}

half3 EnvironmentBRDF(BRDFData brdfData, half3 indirectDiffuse, half3 indirectSpecular, half fresnelTerm,half NV)
{
    half3 c = indirectDiffuse * brdfData.diffuse;
    c += indirectSpecular * EnvironmentBRDFSpecular(brdfData, fresnelTerm,NV);
    return c;
}
