...
namespace UnityEditor.Rendering.Universal
{
    ...
    internal partial class UniversalRenderPipelineAssetUI
    {
        ...
        static void DrawCascadeSliders(SerializedUniversalRenderPipelineAsset serialized, int splitCount, bool useMetric, float baseMetric)
        {
            ...
            Vector4 shadowCascadeSplit2 = Vector4.one;
            if (splitCount == 3)
            ...
            shadowCascadeSplit[2] = Mathf.Clamp(shadowCascadeSplit[2], shadowCascadeSplit[1] + splitBias, shadowCascadeSplit2[0] - splitBias);
            

            shadowCascadeSplit2[0] = Mathf.Clamp(shadowCascadeSplit2[0], shadowCascadeSplit[2]+ splitBias, shadowCascadeSplit2[1] - splitBias);
            shadowCascadeSplit2[1] = Mathf.Clamp(shadowCascadeSplit2[1], shadowCascadeSplit2[0] + splitBias, shadowCascadeSplit2[2] - splitBias);
            shadowCascadeSplit2[2] = Mathf.Clamp(shadowCascadeSplit2[2], shadowCascadeSplit2[1] + splitBias, shadowCascadeSplit2[3] - splitBias);
            shadowCascadeSplit2[3] = Mathf.Clamp(shadowCascadeSplit2[3], shadowCascadeSplit2[2] + splitBias, 1 - splitBias);
            ...
            for (int i = 0; i < splitCount; ++i)
            {
                if (i<3)
                {
                    ...
                    float maximum = 0;
                     if(splitCount>=4)
                    {
                        maximum=   i == 2? shadowCascadeSplit2[0] - splitBias :shadowCascadeSplit[i + 1] - splitBias ;
                    }
                    else
                    {
                        maximum=   i == splitCount - 1 ? 1 : shadowCascadeSplit[i + 1] - splitBias;
                    }
                ...
                {
                    int index = i - 3;
                    float value = shadowCascadeSplit2[index];

                    float minimum = index == 0 ? shadowCascadeSplit[2] + splitBias: shadowCascadeSplit2[index - 1] + splitBias;
                    float maximum = i == splitCount - 1 ? 1 : shadowCascadeSplit2[index + 1] - splitBias;
                    ...
                    if (useMetric)
                    {
                        ...
                        shadowCascadeSplit2[index] = Mathf.Clamp(valueMetric * invBaseMetric, minimum, maximum);
                    }
                    else
                    {
                        float valueProcentage = value * 100f;
                        ...
                        shadowCascadeSplit2[index] = Mathf.Clamp(valueProcentage * 0.01f, minimum, maximum);
                    }
                }
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                switch (splitCount)
                {
                    case 7:
                        ...
                        serialized.shadowCascade8SplitProp.vector4Value = shadowCascadeSplit2;
                        break;
                    case 6:
                        serialized.shadowCascade4SplitProp.vector3Value = shadowCascadeSplit;
                        serialized.shadowCascade7SplitProp.vector3Value = shadowCascadeSplit2;
                        break;
                    case 5:
                        serialized.shadowCascade4SplitProp.vector3Value = shadowCascadeSplit;
                        serialized.shadowCascade6SplitProp.vector2Value = shadowCascadeSplit2;
                        break;
                    case 4:
                        serialized.shadowCascade4SplitProp.vector3Value = shadowCascadeSplit;
                        serialized.shadowCascade5SplitProp.floatValue = shadowCascadeSplit2.x;
            ...
            if (useMetric)
            {
                Vector4 SCS = Vector4.one;
                int index = 0;
                if (splitCount<4)
                {
                    SCS = shadowCascadeSplit;
                    index = splitCount;
                }
                else
                {
                    SCS = shadowCascadeSplit2;
                    index = splitCount-4;
                }
                var lastCascadeSplitSize = index == 0 ? baseMetric : (1.0f - SCS[index - 1]) * baseMetric;
        ...
        static void DrawCascades(SerializedUniversalRenderPipelineAsset serialized, int cascadeCount, bool useMetric, float baseMetric)
        {
            ...
            //添加的额外一个V4 参数用于储存第5到第8级别的级联UI数据
            Vector4 shadowCascadeSplit2 = Vector4.zero;
            
            if(cascadeCount == 8)
            {
                shadowCascadeSplit = serialized.shadowCascade4SplitProp.vector3Value;
                shadowCascadeSplit2 = serialized.shadowCascade8SplitProp.vector4Value;
            }
            else if (cascadeCount == 7)
            {
                shadowCascadeSplit = serialized.shadowCascade4SplitProp.vector3Value;
                shadowCascadeSplit2 = serialized.shadowCascade7SplitProp.vector3Value;
            }
            else if (cascadeCount == 6)
            {
                shadowCascadeSplit = serialized.shadowCascade4SplitProp.vector3Value;
                shadowCascadeSplit2 = serialized.shadowCascade6SplitProp.vector2Value;
            }
            else if (cascadeCount == 5)
            {
                shadowCascadeSplit = serialized.shadowCascade4SplitProp.vector3Value;
                shadowCascadeSplit2.x = serialized.shadowCascade5SplitProp.floatValue;
            }
            ...
            for (int i = 0; i < cascadeCount - 1; ++i)
            {
                ...
                else if (i>=3)
                {
                    ...
                    {
                        //size=0.1f,
                        size = shadowCascadeSplit2[index] - lastCascadePartitionSplit, // Calculate the size of cascade
                        
                        borderSize = 0,
                        cascadeHandleState = ShadowCascadeGUI.HandleState.Enabled,
                        borderHandleState = ShadowCascadeGUI.HandleState.Hidden,
                    };
                    lastCascadePartitionSplit = shadowCascadeSplit2[index];
                }
            ...
            {
                // cascades[3] = new ShadowCascadeGUI.Cascade()
                // {
                //     size = 3 == 0 ? 1.0f : 1 - shadowCascadeSplit[3 - 1], // Calculate the size of cascade
                //     borderSize = serialized.shadowCascadeBorderProp.floatValue,
                //     cascadeHandleState = ShadowCascadeGUI.HandleState.Hidden,
                //     borderHandleState = ShadowCascadeGUI.HandleState.Enabled,
                // };
                cascades[lastCascade] = new ShadowCascadeGUI.Cascade()
                {
                    size = lastCascade == 0 ? 1.0f : 1 - sum, // Calculate the size of cascade
                    borderSize = serialized.shadowCascadeBorderProp.floatValue,
                    cascadeHandleState = ShadowCascadeGUI.HandleState.Hidden,
                    borderHandleState = ShadowCascadeGUI.HandleState.Enabled,
                };
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                else if (cascadeCount == 5)
                {
                    ...
                    serialized.shadowCascade5SplitProp.floatValue =  cascades[0].size + cascades[1].size + cascades[2].size+cascades[3].size;
                }
                else if (cascadeCount == 6)
                {
                    serialized.shadowCascade4SplitProp.vector3Value = new Vector3(
                    ...
                    serialized.shadowCascade6SplitProp.vector2Value = new Vector2(
                        cascades[0].size + cascades[1].size + cascades[2].size + cascades[3].size,
                        cascades[0].size + cascades[1].size + cascades[2].size + cascades[3].size + cascades[4].size
                    );
                }
                else if (cascadeCount == 7)
                {
                    ...
                    serialized.shadowCascade7SplitProp.vector3Value = new Vector3(
                        cascades[0].size + cascades[1].size + cascades[2].size + cascades[3].size,
                        cascades[0].size + cascades[1].size + cascades[2].size + cascades[3].size + cascades[4].size,
                        cascades[0].size + cascades[1].size + cascades[2].size + cascades[3].size + cascades[4].size+cascades[5].size
                    );
                }
                else if (cascadeCount == 8)
                {
                    serialized.shadowCascade4SplitProp.vector3Value = new Vector3(
                        cascades[0].size,
                        cascades[0].size + cascades[1].size,
                    ...
                    serialized.shadowCascade8SplitProp.vector4Value = new Vector4(
                        cascades[0].size + cascades[1].size + cascades[2].size + cascades[3].size,
                        cascades[0].size + cascades[1].size + cascades[2].size + cascades[3].size + cascades[4].size,
                        cascades[0].size + cascades[1].size + cascades[2].size + cascades[3].size + cascades[4].size+cascades[5].size,
                        cascades[0].size + cascades[1].size + cascades[2].size + cascades[3].size + cascades[4].size+cascades[5].size+cascades[6].size
                    );
                }
